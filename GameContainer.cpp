...
void GameContainer::loadFromFile(const std::string& filename) {
    ...
    for (int i = 0; i < numGames; ++i) {
        // вызваем метод класса Game для конкретной игры для считывания ее из файла
        games[i].readFromFile(file);
        if (i < numGames - 1) {
            file.ignore();
        }
    }
...
void GameContainer::saveToFile(const std::string& filename) {
    ...
    for (const auto& game : games) {
        // вызываем метод класса Game для записи конкретной игры в файл
