...
Game* createGameArray(int numGames) {
...
int main() {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            // вызываем метод класса GameContainer для вывода списка игр
            gameContainer.displayGames();
        }
        else if (choice == 2) {
            ...
            Game newGame;
            // вызываем метод класса Game для считываения игры из консоли
            newGame.inputData();
